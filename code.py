...
def main():
    ...
    st.write("""
    The Equity Analysis Calculator helps you evaluate the equity of your URLs...
    """)
    ...
    if st.button("Download Equity Analysis Template XLSX"):
        equity_template_df = pd.DataFrame(columns=[
            "URL", "status_code", "Inlinks", "backlinks", "referring_domains_score", 
            "trust_flow_score", "citation_flow_score", "gsc_clicks_score", "gsc_impressions_score", 
            "unique_pageviews_organic_score", "unique_pageviews_all_traffic_score", 
            "completed_goals_all_traffic_score"
        ])
    ...
    if uploaded_file is not None:
        ...
        if "trust_flow_score" in columns_to_use and "citation_flow_score" in columns_to_use:
            if (norm_data_df["citation_flow_score"] != 0).all(): # Ensure there's no division by zero
